...
namespace AHKExtended.ProfileManager
{
    using System;
    ...
    public class Rule
    {
        ...
        /// <summary>
        ///     Whether to use key sequence instead of single key
        /// </summary>
        [JsonProperty]
        public bool UseKeySequence;

        /// <summary>
        ///     Key sequence to execute when conditions are met
        /// </summary>
        [JsonProperty]
        ...
        public void DrawSettings()
        {
            ...
            // Key mode selection
            ImGui.Checkbox("Use Key Sequence (Combo)", ref this.UseKeySequence);
            ImGuiHelper.ToolTip("Enable this to use a sequence of keys with delays instead of a single key");

            if (this.UseKeySequence)
            {
                // Draw key sequence settings
                this.KeySequence.DrawSettings();
            }
            else
            {
                // Draw single key settings using the new keybind system
                ImGui.Text("Single Key Action:");
                
                // Create a temporary KeyAction for the single key mode
                var tempAction = new KeyAction
                {
                    Key = this.Key,
                    UseCtrl = false,
                    UseAlt = false,
                ...
                if (KeybindCapture.DrawKeybindCapture(-1, tempAction, $"Set Key: {tempAction.GetDisplayString()}"))
                {
                    // Update the single key from the keybind capture
                    this.Key = tempAction.Key;
                    
                    // For single key mode, we ignore modifiers for now to maintain compatibility
                    // Could be enhanced later to support modifiers in single key mode too
                }
                
                ImGui.TextColored(new System.Numerics.Vector4(0.7f, 1.0f, 0.7f, 1.0f), 
                    $"Current key: {this.Key.ToString().Replace("KEY_", "")}");
            }
        ...
        public void Execute(Action<string> logger)
        {
            ...
            {
                if (this.UseKeySequence && this.KeySequence.HasActions)
                {
                    // Execute key sequence 
                    if (!this.KeySequence.IsExecuting)
                    {
                        logger($"Executing key sequence: {this.KeySequence.GetSummary()}");
                        // Execute in a separate task to not block the main thread
                        System.Threading.Tasks.Task.Run(() => this.KeySequence.Execute(logger));
                ...
                {
                    // Execute single key (legacy behavior)
